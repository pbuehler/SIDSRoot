...
/** OscMCTrack.h
 ** Data class for storing Monte Carlo tracks processed by the OscStack.
 ** A OscMCTrack can be a primary track put into the simulation or a
 ** secondary one produced by the transport through decay or interaction.
 **/


#ifndef OscMCTrack_H
#define OscMCTrack_H 1

#include "TObject.h"                    // for TObject
#include "OscDetectorList.h"         // for DetectorId
...
class OscMCTrack : public TObject
{
  ...
  public:
    ...
    OscMCTrack(Int_t pdgCode, Int_t motherID, Double_t px, Double_t py,
                Double_t pz, Double_t x, Double_t y, Double_t z,
                Double_t t, Int_t nPoints);

    /**  Copy constructor  **/
    OscMCTrack(const OscMCTrack& track);


    /**  Constructor from TParticle  **/
    OscMCTrack(TParticle* particle);
...
inline Double_t OscMCTrack::GetEnergy() const
{
...
inline void OscMCTrack::GetMomentum(TVector3& momentum)
{
  momentum.SetXYZ(fPx,fPy,fPz);
}


inline void OscMCTrack::Get4Momentum(TLorentzVector& momentum)
{
  momentum.SetXYZT(fPx,fPy,fPz,GetEnergy());
}


inline void OscMCTrack::GetStartVertex(TVector3& vertex)
{
