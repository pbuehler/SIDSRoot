...
#include "OscMCTrack.h"                // for OscMCTrack
...
// -----   Default constructor   -------------------------------------------
OscStack::OscStack(Int_t size)
  : FairGenericStack(),
    fStack(),
    fParticles(new TClonesArray("TParticle", size)),
    fTracks(new TClonesArray("OscMCTrack", size)),
    fStoreMap(),
    fStoreIter(),
    fIndexMap(),
    fIndexIter(),
...
void OscStack::PushTrack(Int_t toBeDone, Int_t parentId, Int_t pdgCode,
...
TParticle* OscStack::PopNextTrack(Int_t& iTrack)
{
...
TParticle* OscStack::PopPrimaryForTracking(Int_t iPrim)
{
  ...
  if (iPrim < 0 || iPrim >= fNPrimaries) {
    fLogger->Fatal(MESSAGE_ORIGIN, "OscStack: Primary index out of range! %i ", iPrim );
    Fatal("OscStack::PopPrimaryForTracking", "Index out of range");
  }
  ...
  if ( ! (part->GetMother(0) < 0) ) {
    fLogger->Fatal(MESSAGE_ORIGIN, "OscStack:: Not a primary track! %i ",iPrim);
    Fatal("OscStack::PopPrimaryForTracking", "Not a primary track");
  }
...
TParticle* OscStack::GetCurrentTrack() const
{
  TParticle* currentPart = GetParticle(fCurrentTrack);
  if ( ! currentPart) {
    fLogger->Warning(MESSAGE_ORIGIN,"OscStack: Current track not found in stack!");
    Warning("OscStack::GetCurrentTrack", "Track not found in stack");
  }
  return currentPart;
}
// -------------------------------------------------------------------------
...
void OscStack::AddParticle(TParticle* oldPart)
{
...
void OscStack::FillTrackArray()
{

  fLogger->Debug(MESSAGE_ORIGIN, "OscStack: Filling MCTrack array...");
  ...
  for (Int_t iPart=0; iPart<fNParticles; iPart++) {
    ...
    if (fStoreIter == fStoreMap.end() ) {
      fLogger->Fatal(MESSAGE_ORIGIN, "OscStack: Particle %i not found in storage map! ", iPart);
      Fatal("OscStack::FillTrackArray",
    ...
    if (store) {
      OscMCTrack* track =
        new( (*fTracks)[fNTracks]) OscMCTrack(GetParticle(iPart));
      fIndexMap[iPart] = fNTracks;
      // --> Set the number of points in the detectors for this track
      for (Int_t iDet=kOscEsr; iDet<kSTOPHERE; iDet++) {
...
void OscStack::UpdateTrackIndex(TRefArray* detList)
{

  fLogger->Debug(MESSAGE_ORIGIN, "OscStack: Updating track indizes...");
  Int_t nColl = 0;

  // First update mother ID in MCTracks
  for (Int_t i=0; i<fNTracks; i++) {
    OscMCTrack* track = (OscMCTrack*)fTracks->At(i);
    Int_t iMotherOld = track->GetMotherId();
    fIndexIter = fIndexMap.find(iMotherOld);
    if (fIndexIter == fIndexMap.end()) {
      fLogger->Fatal(MESSAGE_ORIGIN, "OscStack: Particle index %i not found in dex map! ", iMotherOld);
      Fatal("OscStack::UpdateTrackIndex", "Particle index not found in map");
    }
  ...
  while( (det = (FairDetector*)fDetIter->Next() ) ) {
    ...
    while ( (hitArray = det->GetCollection(iColl++)) ) {
      ...
      for (Int_t iPoint=0; iPoint<nPoints; iPoint++) {
        ...
        if (fIndexIter == fIndexMap.end()) {
          fLogger->Fatal(MESSAGE_ORIGIN, "OscStack: Particle index %i not found in index map! ", iTrack);
          Fatal("OscStack::UpdateTrackIndex", "Particle index not found in map");
        }
...
// -----   Public method Print  --------------------------------------------
void OscStack::Print(Int_t iVerbose) const
{
  cout << "-I- OscStack: Number of primaries        = "
       << fNPrimaries << endl;
  ...
  if (iVerbose) {
    for (Int_t iTrack=0; iTrack<fNTracks; iTrack++) {
      ((OscMCTrack*) fTracks->At(iTrack))->Print(iTrack);
    }
  }
}
// -------------------------------------------------------------------------
...
void OscStack::AddPoint(DetectorId detId)
{
  Int_t iDet = detId;
...
void OscStack::AddPoint(DetectorId detId, Int_t iTrack)
{
  if ( iTrack < 0 ) { return; }
  Int_t iDet = detId;
...
Int_t OscStack::GetCurrentParentTrackNumber() const
{
...
TParticle* OscStack::GetParticle(Int_t trackID) const
{
  if (trackID < 0 || trackID >= fNParticles) {
    fLogger->Debug(MESSAGE_ORIGIN, "OscStack: Particle index %i out of range.",trackID);
    Fatal("OscStack::GetParticle", "Index out of range");
  }
