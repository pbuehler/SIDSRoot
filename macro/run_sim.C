void run_sim(Int_t nEvents = 100, TString mcEngine = "TGeant4")
{
  ...
  FairModule* cave= new OscCave("CAVE");
  cave->SetGeometryFileName("cave.geo");
  run->AddModule(cave);

  FairModule* magnet = new OscMagnet("Magnet");
  run->AddModule(magnet);

  FairModule* pipe = new OscPipe("Pipe");
  run->AddModule(pipe);
    
  FairDetector* NewDet = new OscEsr("TestDetector", kTRUE);
 ...
 // ------------------------------------------------------------------------
    ...
    // -----   Magnetic field   -------------------------------------------
    // Constant Field
    OscConstField  *fMagField = new OscConstField();
