...
OscEsr::OscEsr()
  : FairDetector("OscEsr", kTRUE, kOscEsr),
    fTrackID(-1),
    fVolumeID(-1),
    fPos(),
    fMom(),
    ...
    fLength(-1.),
    fELoss(-1),
    fOscEsrPointCollection(new TClonesArray("OscEsrPoint"))
{
}

OscEsr::OscEsr(const char* name, Bool_t active)
  : FairDetector(name, active, kOscEsr),
    fTrackID(-1),
    fVolumeID(-1),
    fPos(),
    fMom(),
...
OscEsr::~OscEsr()
{
  if (fOscEsrPointCollection) {
    fOscEsrPointCollection->Delete();
    delete fOscEsrPointCollection;
  }
}

void OscEsr::Initialize()
{
  FairDetector::Initialize();
  FairRuntimeDb* rtdb= FairRun::Instance()->GetRuntimeDb();
  OscEsrGeoPar* par=(OscEsrGeoPar*)(rtdb->getContainer("OscEsrGeoPar"));
}

Bool_t  OscEsr::ProcessHits(FairVolume* vol)
{
  ...
  // Create OscEsrPoint at exit of active volume
  if ( gMC->IsTrackExiting()    ||
    ...
    // Increment number of OscEsr det points in TParticle
    OscStack* stack = (OscStack*) gMC->GetStack();
    stack->AddPoint(kOscEsr);
  }

  return kTRUE;
}

void OscEsr::EndOfEvent()
{

  fOscEsrPointCollection->Clear();

}



void OscEsr::Register()
{

  /** This will create a branch in the output tree called
      OscEsrPoint, setting the last parameter to kFALSE means:
      this collection will not be written to the file, it will exist
      only during the simulation.
  */

  FairRootManager::Instance()->Register("OscEsrPoint", "OscEsr",
                                        fOscEsrPointCollection, kTRUE);

}


TClonesArray* OscEsr::GetCollection(Int_t iColl) const
{
  if (iColl == 0) { return fOscEsrPointCollection; }
  else { return NULL; }
}
...
OscEsrPoint* OscEsr::AddHit(Int_t trackID, Int_t detID,
                                      TVector3 pos, TVector3 mom,
                                      Double_t time, Double_t length,
                                      Double_t eLoss)
{
  TClonesArray& clref = *fOscEsrPointCollection;
  Int_t size = clref.GetEntriesFast();
  return new(clref[size]) OscEsrPoint(trackID, detID, pos, mom,
         time, length, eLoss);
}
